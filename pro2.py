...
from tools import (
    ...
    generate_reservation_conversation,
    determine_intent,
    store_user_info
)
...
def store_user_info(user_input):
    previous_info = st.session_state.user_data
...
# --- User Input ---
user_input = st.chat_input("Ask something about restaurants or reservations...")
...
if user_input:
    ...
    user_info = store_user_info(user_input)
    st.session_state.user_data.update(user_info)
    # Detect intent
    intent = determine_intent(user_input)

        # Generate assistant reply
    required_keys = ["restaurant_name", "name", "contact", "party_size", "time"]
    ...
    if intent == "SELECT":
        history_prompt_30 = " ".join(history_prompt.split()[-30:])
        prompt = f"{schema_prompt}\n\nPrevious response's last 30 words by bot: {history_prompt_30}\n\nCurrent User Input: {user_input}\nGive only SQL query as answer, SQL:"
        response = client.chat.completions.create(
            model="llama3-8b-8192",
            messages=[
        ...
        if all(k in st.session_state.user_data for k in required_keys):
            ...
            try:
                ...
                cursor.execute("SELECT id FROM restaurants WHERE LOWER(name) = LOWER(?)", (user_data["restaurant_name"],))
                ...
                # Debugging output
                # st.write("🔍 Tables Needed:", tables_needed)
                # st.write("📋 Available Tables and Slots:", available)
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, '2025-05-12', ?, ?)
                """, (restaurant_id, user_data["name"], user_data["contact"], user_data["time"], party_size))
                reservation_id = cursor.lastrowid

                for table_id, _ in available:
                    cursor.execute("INSERT INTO reservation_tables (reservation_id, table_id) VALUES (?, ?)", (reservation_id, table_id))

                slot_ids = [slot_id for _, slot_id in available]
                cursor.executemany("UPDATE slots SET is_reserved = 1 WHERE id = ?", [(sid,) for sid in slot_ids])
